...
void MH_Z14A_Init(void)	{
	...
	HAL_UART_Transmit(MH_DEBUG_UART, (uint8_t *)UartBuff, strlen(UartBuff), 100);
...
uint32_t MH_Z14A_Main(void) {
	...
	HAL_UART_Transmit(MH_DEBUG_UART, (uint8_t *)DataChar, strlen(DataChar), 100);

	uint8_t CO2_Packet_Write[9] = {0xff,0x01,0x86,0x00,0x00,0x00,0x00,0x00,0x79};
	HAL_UART_Transmit(MH_Z14A_UART, CO2_Packet_Write, 9, 100);
	HAL_Delay(1000);
...
uint32_t CO2_Read(void) {
	...
	sprintf(DataChar_UART,"uart_recieve %d byte\r\n",(int)rx_count);
		HAL_UART_Transmit(MH_DEBUG_UART, (uint8_t *)DataChar_UART, strlen(DataChar_UART), 100);

	while (rx_count--)
		{
		uint8_t c = RingBuffer_DMA_GetByte(&rx_buffer2);
		sprintf(DataChar_UART,"%d ", (int)c );
		HAL_UART_Transmit(MH_DEBUG_UART, (uint8_t *)DataChar_UART, strlen(DataChar_UART), 100);
